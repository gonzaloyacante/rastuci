...
export async function POST(
    ...
    try {
        ...
        logger.info(`[SyncCA] Querying CA for order ${orderId} using ID ${idToQuery}`);
        ...
        if (
            ...
            logger.info(`[SyncCA] Found new tracking number: ${trackingData.shippingId}`);
        }
            ...
            if (updates.caTrackingNumber) {
                if (order.customerEmail) {
                    await sendTrackingUpdateEmail({
                        ...
                        statusMessage: "¡Tu envío ha sido procesado por Correo Argentino!",
                    });
                    emailSent = true;
                }
            }
