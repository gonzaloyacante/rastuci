      ...
      case 'chat':
        return NextResponse.json({
          ...
          data: {
            ...
            activeSessions: Array.from(mockChatSessions.values()).filter(s => s.status === 'active').length
          }
        });
      ...
      case 'faq':
        return NextResponse.json({
          ...
          data: {
            ...
            activeFaqs: Array.from(mockFAQs.values()).filter(f => f.isActive).length
          }
        });

      case 'stats':
        const totalTickets = mockTickets.size;
        const openTickets = Array.from(mockTickets.values()).filter(t => t.status === 'abierto').length;
        const activeChatSessions = Array.from(mockChatSessions.values()).filter(s => s.status === 'active').length;
        ...
        return NextResponse.json({
          ...
          data: {
            tickets: {
              total: totalTickets,
              open: openTickets,
              resolved: Array.from(mockTickets.values()).filter(t => t.status === 'resuelto').length,
              inProgress: Array.from(mockTickets.values()).filter(t => t.status === 'en_progreso').length
            },
            chat: {
            ...
            faq: {
              total: totalFAQs,
              active: Array.from(mockFAQs.values()).filter(f => f.isActive).length,
              totalViews: Array.from(mockFAQs.values()).reduce((sum, faq) => sum + faq.viewCount, 0)
            }
          }
        });
...
export const DELETE = withAdminAuth(async (request: NextRequest) => {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      // eslint-disable-next-line no-console
      console.error('Error in PUT /api/admin/support:', error);
    }
    return NextResponse.json({
      success: false,
      error: 'Error interno del servidor'
  ...
  const exampleTickets = [
    {
      ...
      messages: [{
        id: `MSG-${String(messageCounter++).padStart(3, '0')}`,
        content: 'No se pudo procesar mi pago con tarjeta de crédito. Aparece un error al finalizar.',
        sender: 'customer' as const,
    ...
    },
    {
      ...
      messages: [
        ...
        {
          id: `MSG-${String(messageCounter++).padStart(3, '0')}`,
          content: 'Lamento escuchar eso. Vamos a procesar un reemplazo inmediato. Por favor, envía fotos del daño.',
          sender: 'admin' as const,
          senderName: 'Soporte Rastuci',
  ...
  const exampleFAQs = [
    {
      ...
      answer: 'Puedes rastrear tu pedido ingresando el código de seguimiento en la página de tracking o desde tu cuenta de usuario.',
      category: 'Envíos',
      isActive: true,
      viewCount: 150,
      helpfulVotes: 45,
    ...
    },
    {
      ...
      answer: 'Aceptamos tarjetas de crédito, débito, MercadoPago, transferencias bancarias y efectivo contra entrega.',
      category: 'Pagos',
      isActive: true,
      viewCount: 200,
      helpfulVotes: 60,
    ...
    },
    {
      ...
      answer: 'Ofrecemos devoluciones gratuitas dentro de los 30 días de la compra. El producto debe estar en condiciones originales.',
