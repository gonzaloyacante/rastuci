...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'sessions') {
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          totalActive: Array.from(activeSessions.values()).filter(s => s.status === 'active').length
        }
      });
    }
    ...
    console.error('Error in GET /api/live-chat:', error);
    return NextResponse.json({
      success: false,
  ...
  try {
    ...
    switch (action) {
      ...
      case 'send-message':
        ...
        if (validatedMessage.sender === 'customer' && shouldTriggerBotResponse(validatedMessage.content)) {
          setTimeout(() => {
            sendBotResponse(validatedMessage.sessionId, validatedMessage.content);
          }, 2000);
        }
    ...
    // Error logging para debugging
    console.error('Error in POST /api/live-chat:', error);
    return NextResponse.json({
      success: false,
      error: 'Error interno del servidor'
    }, { status: 500 });
  ...
  try {
    ...
    switch (action) {
      case 'mark-read':
        ...
        const messages = sessionMessages.get(sessionId) || [];
        const updatedMessages = messages.map((msg: LiveChatMessage) => ({ ...msg, isRead: true }));
    ...
    // Error logging para debugging
    console.error('Error in PUT /api/live-chat:', error);
    return NextResponse.json({
      success: false,
      error: 'Error interno del servidor'
    }, { status: 500 });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Error logging para debugging
    console.error('Error in DELETE /api/live-chat:', error);
    return NextResponse.json({
      success: false,
      error: 'Error interno del servidor'
    }, { status: 500 });
...
function shouldTriggerBotResponse(message: string): boolean {
  const botTriggers = [
    'hola', 'ayuda', 'precio', 'envío', 'horario', 'contacto',
    'producto', 'stock', 'disponible', 'comprar', 'pago'
  ];
...
async function sendBotResponse(sessionId: string, customerMessage: string): Promise<void> {
  const session = activeSessions.get(sessionId);
  if (!session) {
    return;
  }
  ...
  const botResponses = {
    ...
    precio: 'Puedes consultar precios en nuestro catálogo online. Un agente te ayudará con información específica.',
    envío: 'Realizamos envíos a todo el país. Los costos varían según la ubicación. Te conectamos con soporte.',
    horario: 'Nuestro horario de atención es de Lunes a Viernes de 9:00 a 18:00 hs.',
    stock: 'Para consultar stock de productos específicos, un agente te brindará información actualizada.',
    pago: 'Aceptamos múltiples métodos de pago: tarjetas, transferencias y MercadoPago.'
  };

  const lowerMessage = customerMessage.toLowerCase();
  let response = 'Gracias por tu consulta. Un agente te atenderá en breve para brindarte la mejor asistencia.';
