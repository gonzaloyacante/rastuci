...
export async function POST(request: NextRequest) {
  try {
    ...
    } = calculateShippingSchema.parse(body);
    ...
    const zoneInfo = {
      postalCode,
      zone:
        numericCode >= 1000 && numericCode <= 1499
          ? "CABA"
          : numericCode >= 1500 && numericCode <= 1999
            ? "GBA"
            : numericCode >= 2000 && numericCode <= 3999
              ? "Provincias cercanas"
              : "Interior del país",
      coverage: true,
      restrictions: [],
    ...
    logger.error("Error calculating shipping:", { error: error });
