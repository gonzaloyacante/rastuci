...
const knowledgeBase = new Map<string, CategoryData>([
  ['envios', {
    category: 'Envíos',
    keywords: ['envío', 'enviar', 'entrega', 'delivery', 'shipping', 'cuanto tarda', 'demora', 'tiempo'],
    responses: [
      {
        id: 'env-001',
        question: '¿Cuánto tiempo tarda en llegar mi pedido?',
        answer: 'Los envíos dentro de CABA llegan en 24-48hs. Al interior del país entre 3-7 días hábiles según la zona.',
        confidence: 0.95,
      ...
      },
      {
        ...
        answer: 'Envío gratis en compras superiores a $15.000. Para montos menores, el costo varía según destino (entre $800-$2.500).',
        confidence: 0.92,
      ...
      },
      {
        ...
        answer: 'Sí, una vez despachado recibirás un código de seguimiento por email para rastrear tu pedido en tiempo real.',
        confidence: 0.98,
        relatedFAQs: ['env-001']
      }
    ]
  ...
  ['pagos', {
    category: 'Pagos',
    keywords: ['pago', 'pagar', 'tarjeta', 'credito', 'debito', 'mercadopago', 'transferencia', 'efectivo'],
    responses: [
      {
        id: 'pag-001',
        question: '¿Qué métodos de pago aceptan?',
        answer: 'Aceptamos tarjetas de crédito/débito, MercadoPago, transferencia bancaria y efectivo contra entrega en CABA.',
        confidence: 0.97,
      ...
      },
      {
        ...
        answer: 'Sí, ofrecemos hasta 12 cuotas sin interés con tarjetas de crédito participantes y MercadoPago.',
        confidence: 0.94,
        relatedFAQs: ['pag-001']
      },
      {
        ...
        answer: 'Absolutamente. Usamos cifrado SSL y todos los pagos se procesan a través de plataformas certificadas.',
        confidence: 0.96,
        relatedFAQs: ['pag-001']
      }
    ]
  ...
  ['productos', {
    category: 'Productos',
    keywords: ['producto', 'stock', 'disponible', 'talle', 'color', 'medida', 'especificacion'],
    responses: [
      {
        id: 'prod-001',
        question: '¿Cómo sé si hay stock disponible?',
        answer: 'El stock se muestra en tiempo real en cada producto. Si no hay stock, puedes suscribirte para recibir notificaciones.',
        confidence: 0.93,
      ...
      },
      {
        ...
        answer: 'Sí, cada producto de ropa incluye una tabla de medidas detallada. También ofrecemos asesoramiento personalizado.',
        confidence: 0.91,
  ...
  ['devoluciones', {
    ...
    keywords: ['devolucion', 'cambio', 'reembolso', 'defectuoso', 'garantia', 'reclamo'],
    responses: [
      {
        id: 'dev-001',
        question: '¿Puedo devolver un producto?',
        answer: 'Sí, tienes 30 días para devolver productos en perfecto estado. El envío de devolución es gratuito.',
        confidence: 0.96,
      ...
      },
      {
        ...
        answer: 'Desde tu cuenta puedes iniciar el proceso de devolución. Te enviaremos una etiqueta prepaga.',
        confidence: 0.94,
        relatedFAQs: ['dev-001']
      },
      {
        ...
        answer: 'Una vez que recibamos y procesemos la devolución, el reembolso se acredita en 5-10 días hábiles.',
        confidence: 0.92,
  ...
  ['cuenta', {
    ...
    keywords: ['cuenta', 'login', 'password', 'contraseña', 'perfil', 'registro', 'datos'],
    responses: [
      {
        id: 'cta-001',
        question: '¿Cómo creo una cuenta?',
        answer: 'Puedes registrarte con tu email en el botón "Crear Cuenta" o durante el proceso de compra.',
        confidence: 0.95,
        relatedFAQs: ['cta-002']
      },
      {
        ...
        answer: 'Usa la opción "Olvidé mi contraseña" en el login. Te enviaremos un link para restablecerla.',
        confidence: 0.97,
        relatedFAQs: ['cta-001']
      }
    ]
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'stats') {
      const stats = {
        totalQueries: Array.from(faqStats.values()).reduce((sum, stat) => sum + stat.views, 0),
    ...
    if (action === 'categories') {
      const categories = Array.from(knowledgeBase.entries()).map(([key, value]) => ({
        id: key,
        name: value.category,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        suggestionMessage: searchResults.length === 0 
    ...
    console.error('Error in GET /api/ai-faq:', error);
    return NextResponse.json({
      success: false,
  ...
  try {
    ...
    switch (action) {
      ...
      case 'suggest-improvement':
        ...
        console.log('Sugerencia de mejora:', { question, suggestion });
    ...
    // Error logging para debugging
    console.error('Error in POST /api/ai-faq:', error);
    return NextResponse.json({
      success: false,
      error: 'Error interno del servidor'
    }, { status: 500 });
...
async function intelligentSearch(question: string, categoryFilter?: string | null): Promise<SearchResult[]> {
  ...
  for (const [, categoryData] of categories) {
    ...
    const keywordMatches = categoryData.keywords.filter((keyword: string) =>
      ...
      for (const response of categoryData.responses) {
        const questionSimilarity = calculateSimilarity(normalizedQuestion, response.question.toLowerCase());
        const answerSimilarity = calculateSimilarity(normalizedQuestion, response.answer.toLowerCase());
        
        const relevanceScore = Math.max(questionSimilarity, answerSimilarity * 0.7) + 
                              (keywordMatches.length * 0.1);

        if (relevanceScore > 0.3) { // Umbral de relevancia
          results.push({
            ...response,
...
// Registrar consulta para estadísticas
function recordQuery(question: string, category: string | null, foundAnswer: boolean): void {
  // En un sistema real, esto se guardaría en base de datos
  // Debug logging para análisis
  console.log('Query recorded:', { question, category, foundAnswer });
}
...
function getPopularCategories(): Array<{ name: string; views: number }> {
  ...
  for (const [faqId, stats] of faqStats.entries()) {
    ...
    for (const [, categoryData] of knowledgeBase.entries()) {
      ...
      if (faq) {
        const current = categoryStats.get(categoryData.category) || { name: categoryData.category, views: 0 };
...
function getTopFAQs(): Array<FAQResponse & { category: string; views: number; helpfulPercentage: number }> {
  const topFaqs = [];
    ...
    for (const [, categoryData] of knowledgeBase.entries()) {
      ...
      if (faq) {
        topFaqs.push({
          ...
          category: categoryData.category,
          views: stats.views,
          helpfulPercentage: stats.views > 0 ? (stats.helpful / stats.views) * 100 : 0
        });
        break;
      }
    }
...
async function createSupportTicket(data: { question: string; email: string; context: string }): Promise<SupportTicket> {
