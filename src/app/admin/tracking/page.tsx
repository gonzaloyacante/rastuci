...
import { AlertTriangle, Package, Truck, CheckCircle, Clock, Search, RefreshCw, Download } from 'lucide-react';
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.avgDeliveryTime} días</div>
            </CardContent>
          </Card>
        </div>
      )}
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                >
                  {selectedItems.length === filteredTrackingData.length ? 'Deseleccionar todo' : 'Seleccionar todo'}
                </Button>
              </div>
            </CardHeader>
            <CardContent>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left p-2">
                        <input
                          type="checkbox"
                          checked={selectedItems.length === filteredTrackingData.length && filteredTrackingData.length > 0}
                  ...
                  <tbody>
                    {filteredTrackingData.map((item) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b hover:surface-secondary">
                          <td className="p-2">
                            <input
                              type="checkbox"
                          ...
                          <td className="p-2">
                            <div>
                              ...
                              <div className="text-sm text-muted">{item.customerEmail}</div>
                            </div>
                          </td>
                          <td className="p-2">
                            <Badge className={statusColors[item.status as keyof typeof statusColors] || 'surface-secondary text-content'}>
                              <StatusIcon className="h-3 w-3 mr-1" />
                              {item.status}
                            </Badge>
                          </td>
                          <td className="p-2">
                            <span className="text-sm">{item.ocaStatus || 'N/A'}</span>
                          </td>
            ...
            <CardContent>
              <div className="space-y-4">
                {trackingData
                  ...
                  .map((item) => (
                    <div key={item.id} className="flex items-start gap-4 p-4 badge-destructive rounded-lg border">
                ...
                {trackingData.filter(item => item.alertLevel !== 'none').length === 0 && (
                  <div className="text-center py-8 text-muted">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold">{stats?.avgDeliveryTime || 0} días</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Tasa de entrega exitosa</span>
                    <span className="font-bold">
                      {stats ? Math.round((stats.delivered / stats.total) * 100) : 0}%
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Envíos retrasados</span>
                    <span className="font-bold text-error">{stats?.delayed || 0}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">Pendiente: {stats?.pending || 0}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 badge-default rounded"></div>
                    <span className="text-sm">En tránsito: {stats?.inTransit || 0}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 badge-success rounded"></div>
                    <span className="text-sm">Entregado: {stats?.delivered || 0}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 badge-destructive rounded"></div>
                    <span className="text-sm">Retrasado: {stats?.delayed || 0}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
