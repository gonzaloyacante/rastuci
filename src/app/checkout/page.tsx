...
export const metadata: Metadata = {
  ...
  description: 'Finaliza tu compra de forma segura con MercadoPago. Envío gratis y pago 100% seguro.',
  keywords: 'checkout, pago, mercadopago, envío gratis, compra segura, finalizar compra',
  openGraph: {
    title: 'Checkout - Finalizar Compra | Rastuci',
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center">
          {Array.from({ length: 5 }, (_, i) => (
            <div key={`checkout-step-${Math.random()}`} className="flex items-center">
              <div className="w-8 h-8 surface-secondary rounded-full animate-pulse" />
              {i < 4 && <div className="w-16 h-0.5 surface-secondary animate-pulse mx-4" />}
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className="surface border border-muted rounded-lg p-6 sticky top-24">
            ...
            <div className="space-y-3">
              {[...Array(3)].map(() => (
                <div key={`order-line-${Math.random()}`} className="flex justify-between">
