...
export default function ShippingStep({ onNext, onBack }: ShippingStepProps) {
  ...
  const [cachedPostalCode, setCachedPostalCode] = useState<string | null>(null);
  ...
  const calculateShippingByPostalCode = useCallback(async () => {
    ...
    try {
      // Llamar a la API de Correo Argentino con datos reales del carrito
      const response = await fetch("/api/shipping/calculate", {
        ...
        body: JSON.stringify({
          ...
          packages: [
            {
              weight: 1000, // TODO: calcular peso real del carrito
              height: 10,
              width: 20,
              length: 30,
            },
      ...
      // Combinar opciones de CA con retiro en tienda
      ...
      if (!selectedShippingOption && allOptions.length > 0) {
        setSelectedShippingOption(allOptions[0]);
      }
    } catch (error) {
      logger.error("Error al calcular el costo de envío:", { error: error });
      setError(
        "No se pudo calcular el costo de envío con Correo Argentino. Mostrando opciones predeterminadas."
      );
      ...
      if (!selectedShippingOption && availableShippingOptions.length > 0) {
        setSelectedShippingOption(availableShippingOptions[0]);
      }
    } finally {
      setLoading(false);
    }
  ...
  // Cargar opciones de envío SOLO cuando cambia el código postal (OPTIMIZADO CON CACHÉ)
  useEffect(() => {
    // Solo calcular si:
    // 1. Hay código postal
    // 2. No se ha calculado aún O el código postal cambió
    if (
      customerInfo?.postalCode &&
      (!calculatedOnce || cachedPostalCode !== customerInfo.postalCode)
    ) {
      calculateShippingByPostalCode();
      setCachedPostalCode(customerInfo.postalCode);
      setCalculatedOnce(true);
    } else if (!customerInfo?.postalCode && !calculatedOnce) {
