...
export function useTrackingValidation(): UseTrackingValidationResult {
  ...
  const [validationResult, setValidationResult] =
    useState<TrackingValidationResponse | null>(null);
  ...
  const validateTracking = useCallback(
    async (trackingNumber: string): Promise<TrackingValidationResponse> => {
      setIsLoading(true);
      setError(null);
      setValidationResult(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Error desconocido al validar tracking";
...
export async function validateTrackingNumber(
  ...
  const response = await fetch(
    `/api/tracking/validate?number=${encodedNumber}`
  );
