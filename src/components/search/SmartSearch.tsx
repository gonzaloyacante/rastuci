...
export default function SmartSearch({
  ...
  const { data: trendingData } = useSWR('/api/search/trending', async (url: string) => {
  ...
  const fetchSuggestions = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      const productsRes = await fetch(`/api/products/search?q=${encodeURIComponent(searchQuery)}&limit=3`);
      ...
      const matchingCategories = categories.filter((cat: { id: string; name: string }) => 
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (selectedIndex >= 0) {
          ...
          if (showRecent && selectedIndex < currentIndex + recentSearches.length) {
          ...
          if (showTrending && selectedIndex < currentIndex + trendingSearches.length) {
          ...
          >
            ...
            {showTrending && (!query || query.length < 2) && trendingSearches.length > 0 && (
              <div className="p-2 border-t border-surface-secondary">
                ...
                {trendingSearches.map((trendingSearch: string, index: number) => {
