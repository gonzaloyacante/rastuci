...
import type { ProvinceCode, TrackingErrorResponse, TrackingInfo } from "@/lib/correo-argentino-service";
...
export function ShipmentControlCard({ order, onOrderUpdate }: ShipmentControlCardProps) {
    const { getTracking, importShipment, loading: caLoading } = useCorreoArgentino();
    const [loadingTracking, setLoadingTracking] = useState(false);
    const [syncing, setSyncing] = useState(false);
    const [trackingInfo, setTrackingInfo] = useState<TrackingInfo | TrackingInfo[] | TrackingErrorResponse | null>(null);
    ...
    const handleSyncCA = async () => {
        ...
        setSyncing(true);
        try {
            const res = await fetch(`/api/admin/orders/${order.id}/sync-ca`, { method: "POST" });
                    ...
                    {!order.caShipmentId && order.shippingStreet && (
                        <div>
                            ...
                            <Button
                                ...
                                onClick={order.caImportStatus === "ERROR" ? handleRetryImport : handleImportShipment}
                                disabled={caLoading}
                            >
                                {caLoading ? (
                                    <RefreshCw size={16} className="mr-2 animate-spin" />
