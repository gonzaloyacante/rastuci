...
export function MercadoPagoCheckout({ 
  ...
  const handlePayment = async () => {
    ...
    if (!customerInfo.email || !customerInfo.name) {
      show({ type: 'error', title: 'Error', message: 'Completa los datos del cliente' });
      return;
    }
    ...
    try {
      ...
      const metadata = {
        ...
        discountPercent: summary.subtotal > 0 ? summary.discount / summary.subtotal : 0,
    ...
    <div className="space-y-6">
      ...
      <div className="surface rounded-lg border border-muted p-6">
        ...
        <div className="space-y-2 text-sm">
          <p><span className="muted">Nombre:</span> {customerInfo.name}</p>
          <p><span className="muted">Email:</span> {customerInfo.email}</p>
          {customerInfo.phone && (
            <p><span className="muted">Teléfono:</span> {customerInfo.phone}</p>
          )}
          {customerInfo.address && (
            <p><span className="muted">Dirección:</span> {customerInfo.address}</p>
          )}
        </div>
      </div>
      ...
      <div className="surface rounded-lg border border-muted p-4">
        <p className="text-sm font-medium mb-3 text-center">Métodos de pago aceptados</p>
