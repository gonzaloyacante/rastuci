...
export function NotificationManager({ className = '' }: NotificationManagerProps) {
  ...
  const getPermissionStatus = () => {
    switch (permission) {
      case 'granted':
        return { color: 'success', text: t('notifications.permission.granted') };
      case 'denied':
        return { color: 'error', text: t('notifications.permission.denied') };
      default:
        return { color: 'warning', text: t('notifications.permission.default') };
    }
  };
        ...
        <div className="absolute top-full right-0 mt-2 w-80 surface border border-muted rounded-lg shadow-lg z-50">
          <div className="p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold">{t('notifications.settings.title')}</h3>
            ...
            {permission === 'granted' && (
              <div className="space-y-3">
                <h4 className="font-medium">{t('notifications.preferences.title')}</h4>
                
                {Object.entries(preferences).map(([key, enabled]) => (
                  <div key={key} className="flex items-center justify-between">
                    <span className="text-sm">{t(`notifications.types.${key}`)}</span>
                    <button
                      onClick={() => savePreferences({ ...preferences, [key]: !enabled })}
                      className={`
  ...
  return (
    <div className="surface border border-muted rounded-lg p-4 flex items-start gap-3">
      ...
      <div className="flex-1">
        ...
        <p className="text-sm text-muted mb-3">{t('notifications.prompt.description')}</p>
