...
import { useTrackingValidation, type TrackingValidationResponse } from '@/hooks/useTrackingValidation';
import { 
  Search, 
  Loader2, 
  CheckCircle, 
...
export function TrackingValidator({
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error al validar tracking';
          ...
          {validationResult.exists ? (
            <div className="space-y-3">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="font-medium text-success">¡Tracking encontrado!</h3>
                  <p className="text-sm muted mt-1">
                    El número de tracking <span className="font-mono font-medium">{validationResult.trackingNumber}</span> es válido.
                  </p>
                </div>
              </div>

              {/* Detalles del tracking */}
              {showDetails && (validationResult.status || validationResult.description) && (
                <div className="border-t border-muted pt-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {validationResult.status && (
                        <p className={`text-sm font-medium ${getStatusColor(validationResult.status)}`}>
                      ...
                      {validationResult.lastUpdate && (
                        <p className="text-xs muted mt-1">
                          Última actualización: {new Date(validationResult.lastUpdate).toLocaleString('es-ES')}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-muted">
                    <a
                      href={`https://www.oca.com.ar/seguimiento?numero=${validationResult.trackingNumber}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 text-sm text-primary hover:text-primary/80"
                    >
                      <ExternalLink className="w-4 h-4" />
                      Ver seguimiento completo en OCA
                    </a>
                  </div>
                </div>
              )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-medium text-error">Tracking no encontrado</h3>
                <p className="text-sm muted mt-1">
                  {validationResult.error || 'El número de tracking no existe o no se puede validar en este momento.'}
                </p>
