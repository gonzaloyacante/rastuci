  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 p-3 sm:p-3 flex flex-col min-h-[110px] items-center justify-center text-center">
        ...
        <h3
          className={`text-sm sm:text-sm font-semibold mb-1 w-full group-hover:text-primary transition-colors duration-300 line-clamp-2 ${
            displayMode === "image" && img ? "text-white" : "text-foreground"
          }`}
        >
          {category.name}
        </h3>
        ...
        {category.description && (
          <p
            className={`text-xs line-clamp-2 ${
              displayMode === "image" && img ? "text-white/80" : "text-muted"
            }`}
          >
            {category.description}
          </p>
