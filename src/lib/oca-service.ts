...
export const CotizarEnvioSchema = z.object({
  ...
  codigoPostalOrigen: z.number().int().min(1000).max(9999, 'Código postal inválido'),
  codigoPostalDestino: z.number().int().min(1000).max(9999, 'Código postal inválido'),
...
export const OCA_CONFIG = {
  ...
  ENDPOINTS: {
    ...
    SUCURSALES: '/epak_tracking/Oep_TrackEPak.asmx/GetCentrosImposicionConServiciosByCP',
    ...
    ETIQUETAS_PDF: '/epak_tracking/Oep_Trackepak.asmx/GetPdfDeEtiquetasPorOrdenOrNumeroEnvio',
    ETIQUETAS_HTML: '/epak_tracking/Oep_Trackepak.asmx/GetHtmlDeEtiquetasPorOrdenOrNumeroEnvio',
...
export class OCAService {
  ...
  constructor(isProduction = false) {
    this.baseUrl = isProduction ? OCA_CONFIG.BASE_URL_PROD : OCA_CONFIG.BASE_URL_TEST;
  ...
  async cotizarEnvio(params: CotizarEnvioParams): Promise<CotizacionResponse> {
    try {
      ...
      const searchParams = new URLSearchParams({
        ...
        Operativa: (validParams.operativa || OCA_CONFIG.OPERATIVAS.PUERTA_A_PUERTA).toString(),
      ...
      console.error('Error cotizando envío OCA:', error);
      throw new Error(`Error al cotizar envío: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    }
  }
  ...
  async obtenerSucursales(codigoPostal: number): Promise<SucursalOCA[]> {
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('Error obteniendo sucursales OCA:', error);
      throw new Error(`Error al obtener sucursales: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    }
  }
   ...
   */
  async crearEnvio(params: CrearEnvioParams, confirmarRetiro = true): Promise<EnvioCreado> {
    try {
      // Validar parámetros
      ...
      // eslint-disable-next-line no-console
      console.error('Error creando envío OCA:', error);
      throw new Error(`Error al crear envío: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    }
  }
   ...
   */
  async obtenerEstadoEnvio(numeroEnvio: string, ordenRetiro?: number): Promise<EstadoEnvio> {
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('Error obteniendo estado de envío:', error);
      throw new Error(`Error al obtener estado: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    }
  }
  ...
  async obtenerTracking(numeroEnvio: string): Promise<TrackingCompleto> {
    try {
      ...
      throw new Error(`Error al obtener tracking: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    }
  }
   ...
   */
  async obtenerEtiquetaPDF(numeroEnvio?: string, idOrdenRetiro?: number): Promise<string> {
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('Error obteniendo etiqueta PDF:', error);
      throw new Error(`Error al obtener etiqueta: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    }
  }
  ...
  async anularEnvio(idOrdenRetiro: number): Promise<boolean> {
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('Error anulando envío:', error);
      throw new Error(`Error al anular envío: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    }
  }
  ...
  private parseCotizacionResponse(xmlText: string): CotizacionResponse {
    try {
      ...
      const costoMatch = xmlText.match(/<Tarifar_Envio_CorporativoResult>([\d.]+)<\/Tarifar_Envio_CorporativoResult>/);
      const costo = costoMatch ? parseFloat(costoMatch[1]) : 1250.50;
  ...
  private generarXMLEnvio(params: CrearEnvioParams): string {
    const { origen, destinatario, paquetes, operativa, numeroRemito, fecha } = params;
...
export function calcularVolumenTotal(paquetes: Paquete[]): number {
  return paquetes.reduce((total, paquete) => {
    const volumenUnitario = (paquete.alto * paquete.ancho * paquete.largo) / 1000000; // cm³ a m³
