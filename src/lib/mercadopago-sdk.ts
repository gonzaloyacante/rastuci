...
export class MercadoPagoSDK {
  ...
  async createCardToken(cardData: {
    ...
    if (!cardData.cardNumber || !cardData.expiryMonth || !cardData.expiryYear || !cardData.securityCode) {
  ...
  async processPaymentWithSavedCard(cardId: string, amount: number, _installments: number = 1): Promise<{
    id: string;
    status: string;
  ...
  async processPaymentWithNewCard(
    ...
    if (success) {
      ...
      const errorTypes = ['invalid_card', 'insufficient_funds', 'expired_card', 'security_code_invalid'];
...
export const initializeMercadoPagoSDK = async (publicKey: string): Promise<MercadoPagoSDK> => {
  if (!sdkInstance) {
